...
# Ensure no caching UI variable remains; downloads are fresh every time
keep_cached_downloads = False
...
def show_new_results(result: Dict, aspects: List[Dict] = None, speech_emotion_results: Dict = None):
    ...
    if speech_emotion_results:
        with st.container(border=True):
            ...
            try:
                top_text_emotion = sorted(result['emotions'].items(), key=lambda item: item[1], reverse=True)[0][0]
                top_speech_emotion = sorted(speech_emotion_results.items(), key=lambda item: item[1], reverse=True)[0][
                    0]
...
if st.session_state.page == "🏠 Home":
            ...
            if analyze_button_pressed:
                try:
                    ...
                    if isinstance(transcript, str) and not transcript.startswith("❌"):
                        ...
                        st.error(transcript if isinstance(transcript, str) else "❌ Error: Transcription returned unexpected type.")
                    # --- END PATCH ---
    ...
    with tab3:
        st.info("🔗 YouTube: download audio and analyze (no cache).")

        youtube_url = st.text_input("YouTube URL", placeholder="https://www.youtube.com/watch?v=...", key="yt_url_main")
        col_meta, col_opts = st.columns([3, 1])
        with col_opts:
            prefer_ext = st.selectbox("Audio format", ["m4a", "mp3"], index=0)
        ...
        if youtube_url.strip():
            ...
            if meta:
                ...
                st.info("Could not fetch metadata; proceeding to download if you click 'Download & Analyze'.")
        ...
        if st.button("📥 Download & Analyze", use_container_width=True, key="yt_download_btn"):
            if not youtube_url.strip():
                ...
                if not vid:
                    st.error("Could not parse video id from URL. Try a full 'watch?v=' link.")
                else:                    # Download (no cache)

                    audio_path = download_youtube_audio(youtube_url, prefer_ext)
                    ...
                    if isinstance(audio_path, str) and audio_path.startswith("❌"):
                        ...
                        wav_path = os.path.join("temp_files", f"yt_audio_{vid}.wav")

                        conv = convert_to_wav_16k_mono(audio_path, wav_path)

                        if isinstance(conv, str) and conv.startswith("❌"):
                            ...
                            if isinstance(transcript, str) and not transcript.startswith("❌"):
                                ...
                                with st.expander("Transcript", expanded=True):
                                    ...
                                    st.download_button("Download transcript", transcript, f"transcript_{vid}.txt")
                        ...
                        # remove downloaded audio file (no caching)
                        try:
